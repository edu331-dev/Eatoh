...
export default function App() {
  ...
  const { results, status, error, activeQuery, activeCategory, search, browseCategory } =
    useSearch();
  ...
  if (view === "explore" && status === "success") {
    if (activeCategory) {
      gridLabel = `Browsing: ${activeCategory} · ${results.length} recipes`;
    } else if (activeQuery) {
      gridLabel = `${results.length} result${results.length !== 1 ? "s" : ""} for "${activeQuery}"`;
    }
  }
